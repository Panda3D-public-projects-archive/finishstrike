...
As camadas reativa e deliberativa provêem processos denominados de comportamentos. Em termos matemáticos, o comportamento do agente á a função que mapeia qualquer percecpção ou sequência de percepções para uma ação específica \cite{Russel2003} essa função é conhecida como \textit{função de agente}. 
...
\begin{itemize}
\item Redes de uma única camada: existe apenas um nó entre uma entrada e uma saída da rede. A Figura \ref{fig:unicacamada} apresenta dois exemplos de redes de única camada.
...
Nesse trabalho foi estudado um tema particular de visão computacional, que é o de reconhecimento de padrões. Para reconhecer padrões, assim como na maioria dos problemas de visão computacional, é necessario extrair características da imagem. Esse tema será melhor abordado na seção seguinte.

Um padrão em particular que podemos citar são caracteres. O campo de pesquisa, dentro da visão computacional, que se preocupa com o reconhecimento de caracteres é conhecido pelo acrônimo OCR, do inglês, Optical Recognition Caracter. Esse tema será abordado na seçao 2.4.2.
