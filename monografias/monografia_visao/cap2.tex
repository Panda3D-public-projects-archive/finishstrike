...
\item \textit{Thresholding} Local:
 a segunda forma de encontrar o limiar de cada pixel é uma inspeção estatística da sua vizinhança, levando em consideração que os vizinhos de um pixel tendem a possuir maior chance de ter uma iluminação mais uniforme. No método de \textit{thresholding} local, o limiar de um pixel é definido com base nos valores de um número predeterminado de vizinhos do mesmo. Dessa forma, aplica-se uma operação estatística nos valores da vizinhança de um pixel a fim de obter o seu limiar $t$. Essas operações podem ser a média, a mediana ou a média dos valores mínimo e máximo da vizinhança. Na Figura \ref{fig:thLocal} é apresentado o código do algoritmo de\textit{thresholding} local, implementado no Horus, utilizando a média entre os valores da vizinhança.
...
\end{center}
\caption{Código do método de \textit{thresholding} local implementado em linguagem Python.}
\label{fig:thLocal}
\end{figure}
 

No código da Figura \ref{fig:thLocal} os parâmetros passados são: a imagem (image), o tamanho da vizinhança (size) e uma constante que é subtraída da média (con). Na Figura \ref{fig:imgThLocal} é apresentado o resultado da limiarização com o algoritmo de \textit{thresholding} local, utilizando uma vizinhança de tamanho $7\times7$ com um valor de constante igual a $7$ ($size = 7$ e $con = 7$).
...
\end{center}
\caption{Exemplo de situação em que linhas herticais com largura de dois pixels não serão inteiramente removidas.}
\label{fig:grid13}
\end{figure}
\end{itemize}
...
O vetor de características de saída é ilustrado pelo padrão abaixo. A notação $h_j@r_i$ siginifica "número de ocorrências de um tipo de aresta representado pela matriz $h_j$ na região $r_i$", \ref{fig:vetor} 
...
\item Junções: consiste em um ponto que possui pelo menos três pixels pretos na 8-vizinhança. No presente trabalho, considerou-se apenas dois tipos de junções: com três e quatro vizinhos (Figura \ref{fig:junctions} (a),(b)).
...
O \textit{toolkit} Horus também fornece algumas implementações de algoritmos para análise estrututal de caracteres.
